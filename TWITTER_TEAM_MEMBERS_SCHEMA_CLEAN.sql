-- 트위터 팀원 정보 테이블 생성 스크립트 (기존 정책 정리 후 재생성)
-- 실행 날짜: 2025-01-28

-- 기존 정책들 삭제 (존재할 경우)
DROP POLICY IF EXISTS "Anyone can read twitter_team_members" ON twitter_team_members;
DROP POLICY IF EXISTS "Anyone can read twitter_team_activity" ON twitter_team_activity;
DROP POLICY IF EXISTS "Authenticated users can insert twitter_team_members" ON twitter_team_members;
DROP POLICY IF EXISTS "Authenticated users can update twitter_team_members" ON twitter_team_members;
DROP POLICY IF EXISTS "Authenticated users can delete twitter_team_members" ON twitter_team_members;
DROP POLICY IF EXISTS "Authenticated users can insert twitter_team_activity" ON twitter_team_activity;
DROP POLICY IF EXISTS "Authenticated users can update twitter_team_activity" ON twitter_team_activity;
DROP POLICY IF EXISTS "Authenticated users can delete twitter_team_activity" ON twitter_team_activity;

-- 기존 트리거 삭제 (존재할 경우)
DROP TRIGGER IF EXISTS update_twitter_team_members_timestamp ON twitter_team_members;
...
-- 완료 메시지
SELECT 'Twitter 팀원 스키마 설정 완료!' as message;
